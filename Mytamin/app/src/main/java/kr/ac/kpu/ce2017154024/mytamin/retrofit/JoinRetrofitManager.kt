...
import kr.ac.kpu.ce2017154024.mytamin.model.CheckEmailData
...
class JoinRetrofitManager {
    ...
    fun checkEmail(inputemail:String? , completion:(RESPONSE_STATUS,CheckEmailData?) -> Unit){
        val term = inputemail ?:""
                ...
                when(response.code()){
                    200 ->{
                        response.body()?.let {
                            ...
                            val resultData = CheckEmailData(status = status ,result = check )
